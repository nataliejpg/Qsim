...
from qsim.exact.hamiltonians import create_heisenberg_h, create_Huse_h, \
    create_ising_h, create_ising_w_next_nearest_neighbour_h, \
    create_ising_w_all_h
...
def create_heisenberg_u(qubit_num=1, t=0, J=0, h=0, g=0, **kwargs):
    H = create_heisenberg_h(qubit_num=qubit_num, J=J, h=h, g=g, **kwargs)
    l, u = np.linalg.eigh(H)
    U = np.dot(u, np.dot(np.diag(np.exp(-1j * l * t)), dagger(u)))
    return U


def create_ising_u(qubit_num=1, t=0, J=0, Jz=None, h=0, g=0, **kwargs):
    J = Jz or J
    H = create_ising_h(qubit_num=qubit_num, Jz=J, h=h, g=g, **kwargs)
    l, u = np.linalg.eigh(H)
    U = np.dot(u, np.dot(np.diag(np.exp(-1j * l * t)), dagger(u)))
    return U


def create_ising_w_next_nearest_neighbour_u(
        qubit_num=1, t=0, J=0, Jz=None, J2=None, h=0, g=0, **kwargs):
    J = Jz or J
    J2 = J2 or J
    H = create_ising_w_next_nearest_neighbour_h(
        qubit_num=qubit_num, J2=J2, Jz=J, h=h, g=g, **kwargs)
    l, u = np.linalg.eigh(H)
    U = np.dot(u, np.dot(np.diag(np.exp(-1j * l * t)), dagger(u)))
    return U


def create_ising_w_all_u(
        qubit_num=1, t=0, J=0, Jz=None, h=0, g=0, **kwargs):
    J = Jz or J
    H = create_ising_w_all_h(
        qubit_num=qubit_num, Jz=J, h=h, g=g, **kwargs)
    l, u = np.linalg.eigh(H)
    U = np.dot(u, np.dot(np.diag(np.exp(-1j * l * t)), dagger(u)))
    return U
